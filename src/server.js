...
//http://localhost:3007/productos/:nombre
app.get("/productos/:nombre", async (req, res) => {
  ...
  if (producto.length === 0) {
    res.json({
      error: 404,
      message:
        "La búsqueda de productos que contienen el nombre: " +
        nombre_buscado +
...
app.post("/productos", async (req, res) => {
  ...
  if (
    ...
    if (
      ...
      typeof nuevoProducto.categoria !== "string"
    ) {
    ...
    if (
      typeof nuevoProducto.codigo !== "number" ||
      isNaN(nuevoProducto.codigo)
    ) {
  ...
  try {
    ...
    const codigoExistente = await collection.findOne({
    ...
    if (codigoExistente) {
      res
        .status(400)
        .send(
          `El código ${nuevoProducto.codigo} ya existe. Por favor, utiliza un código diferente.`
        );
      return;
    }
